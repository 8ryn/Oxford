...
public :
 ...
 //TBits           *EvtFlags;
   ...
   //=====
   void DumpEvent();
   //B:  Int_t GetTrueReacWithRESModification();
   void SetLv(TLorentzVector & lv, const Int_t ii);
   //B:  void GetParticles(const Int_t numode, Int_t & selp_pid, Int_t & selX_charge, Int_t & selX_pid, Int_t &kNuclearEmission, const Double_t protonMeVthres, const Double_t otherMeVthres, Int_t & nNeutrals);

 readNuWroB():fChain(0){}
   virtual ~readNuWroB();
   virtual void     Init(TTree *tree);
   virtual void     DoAnalysis(const TString filelist, const TString tag, const TString outdir);
};

#endif

#ifdef readNuWroB_cxx

readNuWroB::~readNuWroB()
{
   if (!fChain) return;
...
void readNuWroB::Init(TTree *tree)
{
   ...
   //B: fChain->SetBranchAddress("StruckNucleonPDG", &StruckNucleonPDG, &b_StruckNucleonPDG);
