#define readNuWroB_cxx
#include "readNuWroB.h"
#include "TestUtils.h"

#include <iostream>

void readNuWroB::DumpEvent()
{
...
void readNuWroB::SetLv(TLorentzVector & lv, const Int_t ii)
{
...
void readNuWroB::DoAnalysis(const TString filelist, const TString tag, const TString outdir){
  ...
  Init(tmptree);  //Defined within readNuWroB.h
  ...
  for(Int_t jentry=0; jentry<nentries; jentry++){
    ...
    if(!fChain->GetEntry(jentry)){
      printf("readNuWroB::DoAnalysis GetEntry ends at jentry %d\n", jentry);
      break;
    }

    //Less than 3 particles should be impossible so exit
    if(StdHepN<3){
      printf("readNuWroB ::DoAnalysis StdHepN < 3: %d, impossible, exit\n", StdHepN);
      DumpEvent();
      exit(1);
    }
   ...
   //Sets the output variables for the current entry
    ...
    //Filters events where outgoing lepton is still a neutrino 
    if(StdHepPdg[2]!=13) continue; 
    ...
    TestUtils::SetFullDelta(lIniElec, lIniNuc, lFinLept, lProton);
    TestUtils::SetDeltaPt(lIniElec, lIniNuc, lFinLept, lProton);
    
    TestUtils::fIniElecP = lIniElec.P();
    TestUtils::fIniNucP = lIniNuc.P();
    TestUtils::fFinProtP = lProton.P();
    TestUtils::fFinLeptP = lFinLept.P();
